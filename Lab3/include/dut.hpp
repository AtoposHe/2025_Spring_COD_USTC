...
private:
    ...
    decltype(VTop::commit_inst) & __instr;
...
public:
    inline Dut()
        : __dut_ptr(std::make_shared<VTop>()), __trace_file_ptr(nullptr),
          ...
          __clk(__dut_ptr->clk), __rst(__dut_ptr->rst), __pc(__dut_ptr->commit_pc), __instr(__dut_ptr->commit_inst), __commit(__dut_ptr->commit),
